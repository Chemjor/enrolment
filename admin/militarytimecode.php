		...
		<?php
			...
			echo '<table id="table" class="table table-hover">
				...
				while($num=mysql_fetch_assoc($query))
				{
					...
					$aad=explode("|",$num['days']);//current active day
					$aat=explode("|",$num['schedule']);//current active day

					for ($j=0; $j <$numrow; $j++) { 
						if($j!=$i)
						{
							if(strtoupper($j,$num['subject_code'])==strtoupper($i,$num['subject_code']))
							{
								$flag2=2;
							}
							$od=explode("|", $num['days']);
							$ot2=explode("|", $num['schdule']);
							for ($k=0; $k <count($od) ; $k++) { 
								$d=explode("-", $od[$k]);
								foreach ($d as $nds => $nd) {
									for ($s=0; $s <count($aad) ; $s++) { 
										$ad=explode("-", $aad[$s]);
										foreach ($ad as $key => $naad) {
											if ($nd==$naad) {
												$at1=explode("-", $aat[$s]);
												$t2=explode("-", $ot[$k]);
												if ($at1[0]<=$t2[1]) {
													if ($at1[1]==$t2[1] || $at1[1]<$t2[2] || ($at1[1]<$t2[1] && $at1[1]>$t2[0])) {
														$flag=1;//naa na subject list
													}
												}
												if ($at[0]>$t2[0]) {
													if ($at[0]<$t2[1]) {
					...
					if ($flag==1) {
						$str1="<div class='alert alert-info'>YES</div>";
					}
					if ($flag2==2) {
						$str2="<div class='alert alert-success'>YES</div>";
					}
					echo "
						<tr class='". ($num['block'] ? 'danger' : '') ."'>
						<td>".$num['sub_id']."</td>
...
<?php
	function to12format($t){
		...
		for ($i=0; $i < count($nt); $i++) { 
			...
			$arr=array(DATE("g:i", strtotime($ntarr[0])),DATE("g:i a",strtotime($ntarr[1])));
			$ntarr1[$i]=implode("-",$arr);
		}
		return implode("|", $ntarr1);
	}
