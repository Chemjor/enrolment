...
<div id='loadslip'>
	...
	echo "<table width='100%'><th></th><th>Subject</th><th>Sec</th><th>Description</th><th>Schedule</th><th>Days</th><th>Room</th><th>Units</th><th>Action</th><tr/>";
...
if(isset($_GET['exist'])){
	if($_GET['exist']=='1')
	{
	$error= "<div class='alert-danger' style='margin:10px;text-align:center'>The subject is taken already</div>";
...
<div style="background-color:grey;color:white;height:25px">Subjects to take
...
<script type="text/javascript">
	$('a[href=load_subjects.php?num=$id&id=$studno]').click(function(){  // ... or however you attach to that link
...
<!--end of filter rows-->
		<?php
			$subject_code=$_SESSION['coscode'];
			$query=mysql_query("select * from subject_offered where major=1 and course_id='$subject_code' and  sub_yr='$yrlevel' and semester='$sem' order by subject_code");
			
				if($query)
				{
					 
				echo "<form id='myform'>";
					echo"<table  id='table'  width='100%'><thead><th>SUBJECT</th><th>SECTION</th><th>DESCRIPTION</th><th>SCHEDULE</th><th>DAYS</th><th>ROOM</th><th>UNITS</th></thead>";
			...
			$query=mysql_query("select * from subject_offered where major=0 and  sub_yr='$yrlevel' and semester='$sem' order by subject_code");
				if($query){
				...
				echo"<br>";
					...
					$id=$data["id"];
					$studno=$_SESSION['studno'];
					echo "<tr><td><a href='load_subjects.php?num=$id&id=$studno&yr=$yrlevel'>".$data['subject_code']."</a></td>
					<td>".$data['sections']."</td><td width='50%'>".$data['sub_description']."</td><td>".$data['schedule']."</td>
					<td>".$data['days']."</td><td>".$data['room']."</td><td>".$data['sub_units']."</td></tr>";
